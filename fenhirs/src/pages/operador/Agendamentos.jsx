...
export default function OperadorAgendamentos() {
  ...
  const load = () => api.get('/schedules').then(({ data }) => setItems(data)).finally(() => setLoad(false))
  useEffect(() => { load() }, [])
  ...
  const updateStatus = async (id, status) => {
    setSaving(true)
    try {
      await api.patch(`/schedules/${id}/status`, { status, operatorMessage: opMsg })
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-white mb-1">Gerenciar Agendamentos</h1>
      <p className="text-gray-400 mb-6">Confirme ou recuse as reservas do estande</p>
      ...
      {!loading && filtered.length === 0 && (
        <p className="text-center py-16 text-gray-500">Nenhum agendamento encontrado.</p>
      )}
          ...
          <div key={item.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              <div className="flex-1">
                ...
                {/* Removido: item.User?.name e item.User?.email — a API de agendamentos não retorna esses campos */}
                {item.observation && (
                  <p className="text-gray-400 text-sm italic mt-1">"{item.observation}"</p>
                )}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md">
            <h3 className="text-white font-bold text-lg mb-2">
              {modal.action === 'APPROVED' ? '✓ Confirmar' : '✕ Recusar'} Agendamento
            </h3>
            <textarea
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={() => updateStatus(modal.id, modal.action)} disabled={saving}
                className={`px-6 py-2 text-sm font-semibold rounded disabled:opacity-50 ${

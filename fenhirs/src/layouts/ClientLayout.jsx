...
const navItems = [
  ...
  { to: '/cliente/agendamentos', label: '📅 Meus Agendamentos' },
]
    ...
    <div className="min-h-screen bg-gray-950 flex text-gray-100">
      <aside className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col shrink-0">
        <div className="p-6 border-b border-gray-800">
          ...
          <p className="text-gray-500 text-xs mt-1">Área do Cliente</p>
        </div>

        <div className="p-4 border-b border-gray-800">
          <p className="text-gray-200 text-sm font-semibold truncate">{user?.name}</p>
          <p className="text-gray-500 text-xs truncate">{user?.email}</p>
          <span className="mt-1 inline-block text-xs px-2 py-0.5 rounded bg-blue-900 text-blue-300">Cliente</span>
        </div>
